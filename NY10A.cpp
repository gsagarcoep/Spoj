...
int main()
{
	...
	while(P--)
	{
		...
		std::map< std::string, int> mymap;//={{"TTT",0}, {"TTH",0}, {"THT",0}, {"THH",0}, {"HTT",0}, {"HTH",0}, {"HHT",0},{" HHH",0}};
		mymap["TTT"]=0; mymap["TTH"]=0; mymap["THT"]=0; mymap["THH"]=0; mymap["HTT"]=0; mymap["HTH"]=0; mymap["HHT"]=0;
		mymap["HHH"]=0;
		for(int i=0;i<38;i++)
		{
			string s =str.substr(i,3);
			//cout<<i<<" "<< mymap[s]++<<""<<s<<endl;
			mymap[s]++;
			
		}
		printf("%d %d %d %d %d %d %d %d %d\n",N,mymap["TTT"], mymap["TTH"], mymap["THT"], mymap["THH"], mymap["HTT"], mymap["HTH"], mymap["HHT"],mymap["HHH"]);
	}
	return 0;
}
